...
BuildRocketMQClient() {
  ...
  if [ $test -eq 0 ]; then
    cmake -DLibevent_USE_STATIC_LIBS=ON -DJSONCPP_USE_STATIC_LIBS=ON -DBUILD_STATIC_LIBS=ON -DBUILD_SHARED_LIBS=OFF ..
  else
    if [ $codecov -eq 1 ]; then
      cmake .. -DLibevent_USE_STATIC_LIBS=ON -DJSONCPP_USE_STATIC_LIBS=ON -DBUILD_STATIC_LIBS=ON -DBUILD_SHARED_LIBS=OFF -DRUN_UNIT_TEST=ON -DCODE_COVERAGE=ON
    else
      cmake .. -DLibevent_USE_STATIC_LIBS=ON -DJSONCPP_USE_STATIC_LIBS=ON -DBUILD_STATIC_LIBS=ON -DBUILD_SHARED_LIBS=OFF -DRUN_UNIT_TEST=ON
    fi
  fi
  if [ $verbose -eq 0 ]; then
